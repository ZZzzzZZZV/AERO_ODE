...
class ClipTransform(hk.Module):
  """Transform that clips highest 纵波数 in inputs."""
...
class RedimensionalizeTransform(hk.Module):
  ...
  def __call__(self, inputs: typing.PyTreeState) -> typing.PyTreeState:
    ...
    # Separate physics core variables (with '_physics_' prefix)
    physics_core_inputs = {}
    regular_inputs = {}
    ...
    # Process regular inputs with redimensionalization
    inputs_to_units_mapping = pytree_utils.replace_with_matching_or_default(
    ...
    # Process physics core inputs using the same units as their regular counterparts
    for key, value in physics_core_inputs.items():
      # Remove '_physics_' prefix to find the corresponding regular variable's unit
      ...
      if original_key in self.inputs_to_units_mapping:
        ...
        # Check if unit is a dict (for nested structures like tracers)
        if isinstance(unit, dict):
          # For nested structures, apply dimensionalization recursively
          result[key] = jax.tree_util.tree_map(
              lambda v, u: self.dimensionalize(v, typing.Quantity(u)).m if u is not None else v,
              value, unit
          )
        else:
          # Simple unit, apply directly
          result[key] = self.dimensionalize(value, typing.Quantity(unit)).m
      else:
        # If no unit mapping exists, keep as is
        result[key] = value
