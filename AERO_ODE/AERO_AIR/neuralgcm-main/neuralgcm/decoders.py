...
class LearnedPrimitiveToWeatherbenchDecoder(PrimitiveToWeatherbenchDecoder):
  ...
  def __init__(
      ...
      return_physics_core: bool = False,
      name: Optional[str] = None,
  ):
    super().__init__(
        coords=coords,
    ...
    self.return_physics_core = return_physics_core
  ...
  def __call__(
    ...
    # If return_physics_core is True, store physics core results before adding corrections
    if self.return_physics_core:
      # Store physics core results with '_physics_' prefix
      # Need to expand nested dicts like 'tracers' to match final output structure
      physics_core_dict = {}
      for key, value in wb_on_pressure_dict.items():
        if key == 'diagnostics':
          continue  # Don't duplicate diagnostics
        elif key == 'tracers' and isinstance(value, dict):
          # Expand tracers dict: tracers.specific_humidity -> _physics_specific_humidity
          for tracer_key, tracer_value in value.items():
            physics_core_dict[f'_physics_{tracer_key}'] = tracer_value
        else:
          physics_core_dict[f'_physics_{key}'] = value
    
    wb_on_pressure_dict = jax.tree_util.tree_map(
        add_fn, wb_on_pressure_dict, nodal_outputs
    )
    
    # Merge physics core results into output if requested
    if self.return_physics_core:
      wb_on_pressure_dict = wb_on_pressure_dict | physics_core_dict
...
class DimensionalLearnedPrimitiveToWeatherbenchDecoder(
  ...
  def __init__(
    ...
    super().__init__(
        ...
        return_physics_core=return_physics_core,
        name=name,
    )
    self.redimensionalize_fn = transforms.RedimensionalizeTransform(
        coords,
