...
class NodalVolumeMapping(hk.Module):
  """将节点体积特征的 pytree 映射到给定结构的 pytree。

此模块将输入 pytree 堆叠成形状为 (channel, level, lon, lat) 的数组，并将其传递给 NN 塔。NN 的输出预计具有形状 (n, level, lon, lat)，并解包到具有 output_shapes 结构的 pytree，例如 output_shapes = { 'var_1': jnp.asarray((level, lon, lat)), 'var_2': jnp.asarray((level, lon, lat)), ..., 'var_n': jnp.asarray((level, lon, lat)), }

输入 pytree 的叶子必须具有相同的形状，例如 (1, lon, lat) 或 (level, lon, lat)。要混合形状，请在传递给映射之前进行广播。
  """
