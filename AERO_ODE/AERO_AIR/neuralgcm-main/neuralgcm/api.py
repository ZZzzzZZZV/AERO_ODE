...
class PressureLevelModel:
  ...
  def _from_abbreviated_names_and_tracers(self, outputs: dict) -> dict:
    # Separate physics core variables first
    physics_core_vars = {}
    regular_vars = {}
    ...
    # Process regular variables: expand names and tracers
    ...
    # Process physics core variables: expand names and tracers
    processed_physics_vars = {}
    for k, v in physics_core_vars.items():
      if k == '_physics_tracers' and isinstance(v, dict):
        # Expand _physics_tracers dict
        for tracer_key, tracer_value in v.items():
          processed_physics_vars[f'_physics_{tracer_key}'] = tracer_value
      else:
        # Handle abbreviated names in physics core variables
        # e.g., _physics_t -> _physics_temperature
        original_key = k[len('_physics_'):]  # Remove prefix
        full_name = _FULL_NAMES.get(original_key, original_key)  # Get full name
  ...
  def data_to_xarray_with_physics(
    ...
    """Converts decoded model predictions to two xarray.Dataset objects.
    
    This method separates physics core results (without ML corrections) from
    the full predictions (physics core + ML corrections).
    ...
    Args:
      data: dict of arrays with shapes matching input/outputs or encoded model
        ...
        `([time,] level, longitude, latitude)`,
        where `[time,]` indicates an optional leading time dimension.
        This dict should contain both regular variables and variables with
        '_physics_' prefix (from decoder with return_physics_core=True).
      times: either `None` indicating no leading time dimension on any
        variables, or a coordinate array of times with shape `(time,)`.
      decoded: if `True`, use `self.data_coords` to determine the output
        coordinates; otherwise use `self.model_coords`.
    ...
    Returns:
      A tuple of two xarray.Dataset objects:
        - predictions_ds: Full predictions (physics core + ML corrections)
        - physics_core_ds: Physics core only (without ML corrections)
    """
    coords = self.data_coords if decoded else self.model_coords
    
    # Separate physics core data from full predictions
    physics_core_data = {}
    full_prediction_data = {}
    
    for key, value in data.items():
      if key.startswith('_physics_'):
        # Remove '_physics_' prefix to get original variable name
    ...
    # Also rename abbreviated names to full names for physics core
    # This ensures physics_core_ds has the same variable names as predictions_ds
  ...
  def with_physics_core_output(self, enable: bool = True) -> PressureLevelModel:
    """Returns a new model instance with physics core output enabled/disabled.
    
    When enabled, the decoder will return both the full predictions (physics core
    + ML corrections) and the physics core results separately. This allows
    comparing the physics-only predictions with the ML-corrected predictions.
    
    Args:
      enable: If True, enables physics core output; if False, disables it.
      
    Returns:
      A new PressureLevelModel instance with the specified configuration.
      
    Note:
      This method modifies the gin configuration and may trigger model recompilation.
      For best performance, set this once before running predictions.
    """
    # Modify gin config to add return_physics_core parameter
    modified_config = self.gin_config
    
    # Add or update the return_physics_core parameter in the config
    ...
    # Also handle DimensionalLearnedPrimitiveToWeatherbenchDecoder
    ...
    # Create new model instance with modified config
