...
Run on the server, for example:
  python rmse_ttest.py --workers 4
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--nwp-root",
        type=Path,
        default=Path("/nfs/samba/数据聚变/气象数据/hefang/hrrr_nwp_east_h5"),
    )
    parser.add_argument(
        "--aero-root",
        type=Path,
        default=Path("/nfs/samba/数据聚变/气象数据/NeuralGCM_LAM_predicted_hrrr_rb"),
    )
    parser.add_argument(
        "--neuralgcm-root",
        type=Path,
        default=Path("/nfs/samba/数据聚变/气象数据/zhangjing/NeuralGCM_rb"),
    )
    parser.add_argument(
        "--truth-root",
        type=Path,
        default=Path("/nfs/samba/数据聚变/气象数据/hrrr_440_24var_rb/test/2024"),
    )
    parser.add_argument("--output", type=Path, default=Path("rmse_ttest_results_2024.npz"))
