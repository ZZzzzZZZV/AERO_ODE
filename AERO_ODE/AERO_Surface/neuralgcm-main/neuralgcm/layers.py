...
该网络是 Transformer 架构的简单变体。它可配置为表示编码器和解码器块。与其他层相反，此模块接受其他可选参数：`latents` 和
`positional_encoding`，使其能够表示具有更复杂依赖结构的计算。默认情况下，这些参数的值为 `None`，在这种情况下，网络使用 `inputs` 并在整个过程中执行自注意力计算。如果提供了 `latents`，则它们用于所有注意力块的键和值计算。如果提供了 `positional_encoding`，则它用于生成注意力块中的第一组查询。
此外，此模块支持带门控机制的扩展，通常类似于 https://arxiv.org/pdf/1910.06764.pdf 中的 GTrXL Transformer。

属性：
output_size：模块输出中所需的通道数。
latent_size：潜在表示大小。必须能被 `num_heads` 整除。
n_layers：网络中的转换器块数。
num_heads：每个注意层中的注意头数。
key_size：用于计算注意分数的键/查询向量的大小。
widening_factor：每个块末尾的密集层中的扩展因子。
activation：在线性变换之间应用的激活函数。
final_activation：可选的激活函数，应用于输出。
input_projection_net：用于将输入投影到初始潜在表示中的网络或层。如果设置为 `None`，则完全跳过输入投影（仅当输入大小 == latent_size 时才有可能）。
skip_final_projection：是否跳过最终投影层。如果设置为 `True`，则请求的 `output_size` 必须等于 `latent_size`。
gating_module：用于组合残差连接和密集更新的门控机制。默认为残差连接。
name：模块的可选名称。
  """
