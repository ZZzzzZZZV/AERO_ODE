...
"""定义 AbstractModel API、标准实现和辅助函数。"""
...
def get_model_specs(
  ...
  Args:
    ...
    reference_datetime_str: reference datetime for which nondimensionalized time
      is set to 0. Must be None if already defined in aux_features.
       参数：
    data_coords：数据中表示状态的坐标系。
    physics_specs：物理常数和自定义单位的定义。
    aux_features：数据集附带的辅助特征。
    model_time_step：模型中外部时间步长的持续时间，即
      在单个 Model.Advance 调用中推进状态的时间。
    custom_coords：模型使用的可选坐标系，而不是
      data_coords。
    reference_temperature：用于 sigma 坐标的参考温度。
      如果已在 aux_features 中定义，则必须为 None。 默认值 288
      如果为 None，则为 none，则为 not aux_features。
    reference_datetime_str：引用 datetime for which nondimensionalized time
      设置为 0。如果已在 aux_features 中定义，则必须为 None。 Default
